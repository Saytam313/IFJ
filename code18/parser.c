...
//    printf("token type je %d    %s\n", token->type, token->val.c);
    if(token->type == token_id || token->type == token_id_function){
...
//    printf("statementlist token type je %d\n", token->type);
    ...
    if(token->type == token_id || token->type == token_id_function  ||  token->type == token_print  || token->type == token_inputf
...
//    printf("start STATEMENT\n");
    if(token->type == token_eol){
        ...
        return true;
    } else if(token->type == token_id || token->type == token_id_function){
//        printf("statement id\n");
...
//                printf("prosel jsem prog_def\n");
    ...
    } else if(token->type == token_eol || token->type == token_nic) {
        return PROG();
...
//    f = fopen("soubor.txt", "r");
    ...
    builtin_chr_generate();
    builtin_ord_generate();
    builtin_substr_generate();
